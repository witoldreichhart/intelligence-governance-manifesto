# Twelve Principles of Intelligence Governance, with Four Integrity Preconditions
...
These **twelve principles** govern the substrate from which organizational intelligence emerges, supplemented by **four integrity preconditions** (Part III) that protect the substrate when it is treated as load-bearing infrastructure for AI agents. The preconditions are numbered Principles 13–16 for stable cross-reference, but they are preconditions in role: an IGM substrate cannot be claimed operational without them. Knowledge — structured claims with provenance, epistemic tier, and scope — is what the system holds. Organizational intelligence — the institutional capacity to perceive relevant patterns, infer consequences, and select action opportunities aligned with purpose — is what the system enables. The principles maintain the substrate; the preconditions protect it. What emerges on top depends on how well the substrate is built, maintained, and defended.
...
## Part III — Substrate-Integrity Preconditions

*What protects the substrate from epistemic, adversarial, architectural, and emergent failure.*

The twelve principles in Parts I and II describe the architecture of governed intelligence and how the organisation must run it. The four **integrity preconditions** in this part — numbered Principles 13–16 for stable cross-reference — address what protects that substrate when it is treated as load-bearing infrastructure for AI agents: the difference between agreement and reality, the substrate as an attack surface, the architectural assumptions the IGM relies on but does not provide, and the emergent behaviour produced by multi-agent systems reading and writing the same graph. They are called preconditions because they are conditions under which the twelve principles can be operationally relied upon — without them, the principles still describe the substrate but the substrate cannot be trusted as load-bearing. These preconditions wire IGM into the AEM principles it depends on (AEM P3 architecture, AEM P8 evaluations, AEM P10 emergence and containment) without restating them.
...
**Minimum bar:** Cryptographic provenance signatures on all claims at intake; signature verification on every Apply-stage retrieval. Write-path access controls separated from read-path — the credentials that allow an agent to read the graph cannot, by themselves, write or promote claims. Continuous integrity monitoring with tamper alerts on claim content, provenance metadata, and epistemic-tier transitions. Indirect-prompt-injection scanning is part of Ingest, not optional. Quarterly red-team of the graph itself (not just of agents that consume it), exercising at least the six attack classes named above. A **substrate-security function** is accountable for the integrity programme and reports through an independent line — typically the CISO or equivalent. The function is **not** a fifth governance authority: substrate security is a defence discipline distinct from the epistemic discipline of the Semantic, Assertion, Inference, and Revision authorities, and conflating the two corrupts both. Its mandate is operational, not governance: during integrity incidents it holds defined override authority — force-quarantine of compromised claims, freeze on epistemic-tier promotion, revocation of authority credentials, write-path lockdown — exercised under documented incident-response procedures with post-hoc audit by the four authorities and second-line oversight. Outside incidents, the security function audits and consults the four governance authorities; it does not exercise their epistemic prerogatives.
...
| v1.5 | Title / Preamble / Part III | Reframed as **"twelve principles plus four integrity preconditions"**. Title changed from "Sixteen Principles..." to "Twelve Principles ... with Four Integrity Preconditions". Preamble clarifies the principles/preconditions split: the twelve maintain the substrate; the four protect it. Part III renamed from "Principles of Substrate Integrity" to "Substrate-Integrity Preconditions" with explicit framing that P13–P16 are preconditions in role and principles only in numbering. Stable P-numbers retained so all cross-references continue to resolve. | Author decision 2026-05-03 (commercial framing: preserve "the twelve principles" as the headline while keeping P13–P16 normative) |
| v1.5 | P14 | Resolved DRAFT note. Substrate-security is now specified as a **separate function reporting through an independent CISO line, not a fifth governance authority**. Body of the minimum bar elaborates: substrate security is a defence discipline distinct from the epistemic disciplines of the four authorities; the function holds operational override during integrity incidents (force-quarantine, freeze on tier promotion, credential revocation, write-path lockdown) under documented incident-response procedures with post-hoc audit; outside incidents it audits and consults the four authorities without exercising their epistemic prerogatives. DRAFT note removed. | Author decision 2026-05-03 (separation of governance vs. defence; CISO-line independence preserved) |
| v1.5 | Stack framing | Across positioning.md, README.md, and manifesto.md, retired "one layer in the stack" / "parallel companion" prose. Replacement framing: the manifestos govern *different surfaces*, connected by **structural dependency, not hierarchy**. The dependency direction (AEnt-M depends on IGM; AEM is inherited) is unchanged; what changes is that altitude language ("layer", "above", "below") is no longer used for the IGM↔AEnt-M relationship. | Author decision 2026-05-03 (state dependency without primacy) |
